...
export function ExerciseForm({ exercise, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!name.trim()) return
    setSaving(true)
    const data = { name: name.trim(), muscleGroups, tags, video: video.trim() || undefined, instructions: instructions.trim() }
    if (exercise?.id) {
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <input value={name} onChange={e => setName(e.target.value)}
              className="w-full bg-slate-800 text-white rounded-lg px-3 py-2 border border-slate-700" required />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {EXERCISE_TAGS.map(t => (
                <button key={t} type="button" onClick={() => setTags(toggle(tags, t))}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    tags.includes(t)
                      ? 'bg-emerald-900/40 border-emerald-600 text-emerald-300'
                      : 'bg-slate-800 border-slate-700 text-slate-400'
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1">URL del video (opcional)</label>
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1">Instrucciones</label>
